...
extern uint8 blink;
static void SimpleBLEPeripheral_clockHandler_USER(UArg arg);
static void SimpleBLEPeripheral_performPeriodicTask_USER(void);
...
#define SBP_PERIODIC_EVT_PERIOD_USER          1000
...
#define SBP_PERIODIC_EVT_USER                 0x1000


extern uint8 count_blink;
extern PIN_Handle ledPinHandle;
extern uint_t currentOutputVal;
...
Clock_Struct periodicClock;
 Clock_Struct periodicClock_USER;
 Clock_Struct periodicClock_LINK_LOSS;
...
#ifdef FEATURE_OAD_USER
void SimpleBLEPeripheral_processOadWriteCB_USER(uint8_t event, uint16_t connHandle,
                                           uint8_t *pData);
#endif //FEATURE_OAD_USER
...
static void SimpleBLEPeripheral_init(void)
{
  ...
  //add
  Util_constructClock(&periodicClock_USER, SimpleBLEPeripheral_clockHandler_USER,
                        SBP_PERIODIC_EVT_PERIOD_USER, 0, false, SBP_PERIODIC_EVT_USER);
...
//  LINK_LOSS_AddService();
#ifndef FEATURE_OAD
  SimpleProfile_AddService(GATT_ALL_SERVICES); // Simple GATT Profile
  Batt_AddService();
  ImmedAlert_AddService();
  LINK_LOSS_AddService();
...
static void SimpleBLEPeripheral_taskFxn(UArg a0, UArg a1)
{
  ...
  for (;;)
  {
    ...
    //add
    if (events & SBP_PERIODIC_EVT_USER)
       {
         events &= ~SBP_PERIODIC_EVT_USER;

         Util_startClock(&periodicClock_USER);

//          Perform periodic application task
         SimpleBLEPeripheral_performPeriodicTask_USER();
       }
    ///
#ifdef FEATURE_OAD
...
static void SimpleBLEPeripheral_performPeriodicTask_USER(void)
{
    ...
    currentOutputVal =  PIN_getPortOutputValue(ledPinHandle);

    /* Toggle the LEDs, configuring all LEDs at once */
    PIN_setPortOutputValue(ledPinHandle, ~currentOutputVal);

    user_print("fffff\r\n");
    if(count_blink == 20)
    {
        count_blink = 0;
        Util_stopClock(&periodicClock_USER);
    }
}
...
//    user_print("\n immediate:  \r\n");
//    print_number(blink);
//       if(blink == 0x05||blink == 0x00)
//       {
//           user_print("\n immediate:  \r\n");
//         for(dem=0;dem<11;dem++)
//         {
...
//////////////add function
bool HeartRate_toggleAdvertising(void);
bool HeartRate_toggleAdvertising(void)
{
  uint8_t advState;
