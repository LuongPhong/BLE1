...
extern Clock_Struct periodicClock_LINK_LOSS;
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        disconnect = 0x00;
        PIN_setOutputValue(ledPinHandle, IOID_7, 0);
//        PIN_setOutputValue(ledPinHandle, IOID_7, 0);
        Util_stopClock(&periodicClock_LINK_LOSS);
        if (pPkt->hdr.status == SUCCESS)
        {
    ...
    case GAP_LINK_TERMINATED_EVENT:
      {
          user_print("disconnect");
          if(disconnect == 0x02)
          {
          Util_startClock(&periodicClock_LINK_LOSS);
          }
